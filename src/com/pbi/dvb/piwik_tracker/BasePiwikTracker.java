package com.pbi.dvb.piwik_tracker;
...
public abstract class BasePiwikTracker implements PiwikTracker
{
    protected static final int STATUS_FREE = 10000;
    
    protected static final int STATUS_USING = 10001;
    
    protected Integer TRACKER_STATUS = STATUS_FREE;
    ...
    private static final String TAG = "BasePiwikTracker";
     ...
     * Debug only.
     */
    protected String debugAppendUrl = "";
    ...
    protected Map<BrowserPlugins, Boolean> plugins = new EnumMap<BrowserPlugins, Boolean>(BrowserPlugins.class);
    
    /**
     * Custom data per pageview.
     */
    protected Map<String, String> pageCustomVar = new HashMap<String, String>(SimplePiwikTracker.MAX_CUSTOM_VARIABLES);
    
    /**
     * Custom data per visitor.
     */
    protected Map<String, String> visitorCustomVar = new HashMap<String, String>(
        SimplePiwikTracker.MAX_CUSTOM_VARIABLES);
    
    protected Date forcedDatetime;
     ...
     */
    protected String pageCustomData;
    
    /**
     * If the visitor custom variables map is empty use this data.
     */
    protected String visitorCustomData;
    ...
    protected URL urlReferrer;
    
    protected HttpURLConnection longTermConnection;
     ...
     */
    public BasePiwikTracker(final String apiUrl)
        throws PiwikException
    {
     ...
     * @throws PiwikException
     */
    public BasePiwikTracker(final int idSite, final String apiUrl, final HttpServletRequest request)
        throws PiwikException
    {
        this(apiUrl);
        this.idSite = idSite;
     ...
     */
    public final void readRequestInfos(final HttpServletRequest request)
        throws PiwikException
    {
        if (request != null)
        {
     ...
     * @throws PiwikException
     */
    public final void setApiurl(final URL apiurl)
        throws PiwikException
    {
        if (apiurl == null)
        {
     ...
     * @throws PiwikException
     */
    public final void setUrlReferrer(final String urlReferrer)
        throws PiwikException
    {
        try
        {
     ...
     */
    public final int setPageCustomVariable(final String name, final String value)
        throws PiwikException
    {
     ...
     *             maximum variable length
     */
    public final int setVisitorCustomVariable(final String name, final String value)
        throws PiwikException
    {
     ...
     * @throws PiwikException
     */
    public final void setLocalTime(final String time)
        throws PiwikException
    {
        Date date = null;
        if (time != null)
    ...
    public final String getGeneralQuery()
    {
        ...
        final String withForcedTimestamp =
            this.addParameter(withIp,
                "cdt",
        ...
        final String withRand =
            this.addParameter(withPageCustomVar, "r", String.valueOf(random.nextDouble()).substring(2, 8));
     ...
     */
    private URL makeURL(final String queryString)
        throws MalformedURLException
    {
    ...
    public final URL getPageTrackURL()
    {
        URL output = null;
        ...
        try
        {
            ...
            output = this.makeURL(globalQuery);
        }
        catch (final MalformedURLException e)
        {
            LogUtils.e(TAG, "Error while building track url");
     ...
     * @throws PiwikException
     */
    public final ResponseData sendRequest(final URL destination)
        throws PiwikException
    {
    ...
    /**
     * Connect to the PIWIK-Server with the url assigned in variable of destination.
     * 
     * We need to keep the connection available before we disconnect it..
     * 
     * But at this moment, the affect is not sure yet...
     * 
     * @param destination the built request string.
     * 
     * @return <code>true</code> if the we connect to the piwik-server successfully...This means not timeout, not
     *         unknown host and any other exception happened...
     */
    // public boolean connectToServer(final URL destination)
    public synchronized boolean connectToServer()
    {
        TRACKER_STATUS = STATUS_USING;
        
        ResponseData responseData = null;
        
        URL destination = getPageTrackURL();
        
        if (destination != null)
        {
            try
            {
                LogUtils.e(TAG, "try to open piwik request url: {" + destination + "}");
                
                if (longTermConnection != null)
                {
                    LogUtils.e(TAG,
                        "before connect to server, the longTermConnection is not null... longTermConnection = "
                            + longTermConnection.getURL());
                    
                    longTermConnection.disconnect();
                    
                    longTermConnection = null;
                }
                
                longTermConnection = (HttpURLConnection)destination.openConnection();
                longTermConnection.setInstanceFollowRedirects(false);
                longTermConnection.setRequestMethod("GET");
                longTermConnection.setConnectTimeout(600);
                longTermConnection.setRequestProperty("User-Agent", userAgent);
                longTermConnection.setRequestProperty("Accept-Language", language);
                longTermConnection.setRequestProperty("Connection", "Keep-Alive");
                longTermConnection.setUseCaches(false);
                
                if (requestCookie != null)
                {
                    longTermConnection.setRequestProperty("Cookie",
                        requestCookie.getName() + "=" + requestCookie.getValue());
                }
                
                // change the status of this track...
                TRACKER_STATUS = STATUS_USING;
                
                longTermConnection.connect();
                
                responseData = new ResponseData(longTermConnection);
                ...
                // if (connection.getResponseCode() != HttpServletResponse.SC_OK)
                if (longTermConnection.getResponseCode() != HttpURLConnection.HTTP_OK)
                {
                    LogUtils.e(TAG, "BasePiwikTracker.connectToServer:: connection.getResponseCode() = "
                        + longTermConnection.getResponseCode() + "connection.getResponseMessage() = "
                        + longTermConnection.getResponseMessage());
            ...
            {
                LogUtils.e(TAG, "BasePiwikTracker.connectToServer:: SocketTimeoutException occur... url = "
                    + destination);
            ...
            {
                LogUtils.e(TAG, "BasePiwikTracker.connectToServer:: UnknownHostException occur... url = " + destination);
            }
            catch (final IOException e)
            {
                LogUtils.e(TAG, "BasePiwikTracker.connectToServer:: SocketTimeoutException occur...");
        ...
        {
            LogUtils.e(TAG,
                "BasePiwikTracker.connectToServer:: the destination passed in should not be null but it is...");
            
            return false;
        }
        
        LogUtils.e(TAG,
            "BasePiwikTracker.connectToServer:: connect success, we suppose it's not timeout here and no exception happened...");
        
        return true;
    }
    
    /**
     * Disconnect from the PIWIK-Server
     * 
     * @param null
     * 
     * @return null
    ...
    public synchronized void disconnectFromServer()
    {
        ...
        {
            LogUtils.e(TAG,
                "Error!!! BasePiwikTracker.disconnectFromServer:: the longTermConnection should not be null but it is...");
        }
    }
    
    public boolean isAvailable()
    {
        if (TRACKER_STATUS == STATUS_USING)
        {
            return false;
        }
        else if (TRACKER_STATUS == STATUS_FREE)
        {
            return true;
        }
        else
        {
            LogUtils.e(TAG, "unknwon status TRACKER_STATUS = " + TRACKER_STATUS);
