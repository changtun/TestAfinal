    ...
    if(err == TM_OK || err == HCDP_ERR_ERR_NO_INSTANCES)
    {
        ...
        else 
        {
            ...
            if(err == TM_OK)
            {
                ...
                tmDBGPRINTEx(DEBUGLVL_INOUT, "tmbslHCDP__Open(0x%08X)\n", tUnit);
 ...
 )
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDP_Reset(0x%08X)\n", tUnit);
	...
	tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmbslHCDP_Reset(0x%08X) failed.", tUnit));
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDP_StartLock(0x%08X)\n", tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDP_GetLockStatus(0x%08X)\n", tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDP_SetSR(0x%08X)n", tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDP_SetFECMode(0x%08X)\n", tUnit);
...
)
{
    ...
    if (err == TM_OK)
    {
        if (sLockInd.bDemod && sLockInd.bFEC)
        {
            ...
            if (err == TM_OK)
            {
				//printf("%s uUncors:%d,pObj->uPreviousUncors:%d %d \n",__FUNCTION__,uUncors,pObj->uPreviousUncors,__LINE__);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_SetMFFilter(0x%08X)\n", pObj->tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_ResetCdp(0x%08X)\n", pObj->tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_CheckFECLocked(0x%08X)\n", pObj);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_StartBER(0x%08X)\n", pObj);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_ExtendSymbolRate(0x%08X)\n", pObj);
    ...
    if( (pObj->eHwCdpSample == HCDP_SAMPLE_ES2) && (err == TM_OK) ) /* temporary: to be adapted when ES1 is removed */
    {
        if( (eMode != HCDP_ExtendSymbolRateModeDisable) &&
            (((pObj->sCurrentChannel.eFECMode == tmFrontEndFECModeAnnexA) ||
              (pObj->sCurrentChannel.eFECMode == tmFrontEndFECModeAnnexC))
        ...
        else
        {
            ...
            /* Do not program registers if extend SR is already disable */
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_INOUT, "iHCDP_Wait(0x%08X)\n", pObj->tUnit);
