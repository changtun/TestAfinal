...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        else
        {
            ...
            if (err == TM_OK)
            {
                err = tmddTDA18250SetpdAGC1b(tUnit, tmddTDA18250_PD_NotActivated/*tmddTDA18250_PD_Activated*/);
  //              err = tmddTDA18250SetpdAGC1b(tUnit, tmddTDA18250_PD_Activated);
            ...
            if( pObj->ePowerTransitionMode == tmTDA18250_PowerTransitionMode_Smooth )
            {
                ...
                if (err == TM_OK)
                {
                    ...
                    err = tmddTDA18250SetpdDETECT1(tUnit, tmddTDA18250_PD_NotActivated/*tmddTDA18250_PD_Activated*/);
                    tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmddTDA18250SetpdDETECT1(0x08X) failed.", pObj->tUnit));
                }
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        //printf("FUN:%s,LINE:%d,Agc1Multi:0x%x\n",__FUNCTION__,__LINE__,Agc1Multi); 
		//Int32 b  = TDA18250CalcPow(10, Lvl_Div10[tmTDA18250_XDIV10_AGC1Multi][tmTDA18250_XDIV10_SUM]);
        //printf("FUN:%s,LINE:%d,b:%d, *pPowerLevel:%d\n",__FUNCTION__,__LINE__,b,*pPowerLevel); 
        *pPowerLevel -= Agc1Multi / ( TDA18250CalcPow(10, Lvl_Div10[tmTDA18250_XDIV10_AGC1Multi][tmTDA18250_XDIV10_SUM]) );
		//printf("FUN:%s,LINE:%d,pPowerLevel:%d\n",__FUNCTION__,__LINE__,*pPowerLevel); 
    }
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        {
            /* Search the new Hybrid Band used */
            err = TDA18250RF2BandIndex(tUnit, uRfFreq, &uIndex);//printf("FUN:%s,LINE:%d,uIndex:%d\n",__FUNCTION__,__LINE__,uIndex);
            tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "TDA18250RF2BandIndex(0x%08X) failed.", tUnit));
        }
        ...
        */
        ...
        if (err == TM_OK)
        {
			...
			//printf("FUN:%s,LINE:%d,PowerLevel:%d,uRSSIValue:%d\n",__FUNCTION__,__LINE__,PowerLevel,uRSSIValue);
        }
        ...
        if (err == TM_OK)
        {
			...
			//printf("FUN:%s,LINE:%d,TempPowerLevel:%d\n",__FUNCTION__,__LINE__,TempPowerLevel);
			{
				//Int32 a = TDA18250CalcPow(10, Lvl_Div10[tmTDA18250_DIV10_RF][tmTDA18250_XDIV10_SUM]);
				//printf("FUN:%s,LINE:%d,a:%d\n",__FUNCTION__,__LINE__,a);
			}
            PowerLevel -= TempPowerLevel / ( TDA18250CalcPow(10, Lvl_Div10[tmTDA18250_DIV10_RF][tmTDA18250_XDIV10_SUM]) );
			//printf("FUN:%s,LINE:%d,PowerLevel:%d\n",__FUNCTION__,__LINE__,PowerLevel);
        }
        ...
        if (err == TM_OK)
        {
            switch(AGC1_Type)
            {
                case tmTDA18250_AGC1_MasterSingle:
                    err = TDA18250LvlAgc1Contribution(uAGC1_Current, uRfFreq, &PowerLevel);//printf("FUN:%s,LINE:%d\n",__FUNCTION__,__LINE__);
                    tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "TDA18250LvlAgc1Contribution(0x%08X) failed.", tUnit));

                    break;

                case tmTDA18250_AGC1_MasterMulti:
                    err = TDA18250LvlAgc1MasterMultiContribution(uAGC1_Current, uRfFreq, &PowerLevel);//printf("FUN:%s,LINE:%d,PowerLevel:%d\n",__FUNCTION__,__LINE__,PowerLevel);                    
		...
		{
			//printf("TempPowerLevel:%d,PowerLevel:%d\n",TempPowerLevel,PowerLevel);
		}
        /* return dBmV */
        if (err == TM_OK)
        {
