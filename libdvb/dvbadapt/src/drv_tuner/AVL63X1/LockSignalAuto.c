...
#define	DVBC_IF_INPUT_FREQUENCY     6*1000000				/*IF signal frequency*/    
...
#define	TUNER1_ADDRESS				0x60					/*tuner I2C address*/ 
#define TUNER1_CLOCK_SPEED_KHZ      400                     /*I2C clock*/
...
AVL63X1_ErrorCode lock_tuner(struct AVL_Tuner *ptuner)
{
	...
	SemcoMxL601_Initialize(ptuner);
	/*-------------------------------------*
	 |lock tuner using specified function  |
	 *-------------------------------------*/
	return_code = SemcoMxL601_Lock(ptuner);
	...
	return_code = SemcoMxL601_GetLockStatus(ptuner);
	if(return_code != AVL63X1_EC_OK)
	{
...
AVL63X1_ErrorCode get_lock_status(int *lock_status)
{
	...
	printf("get_lock_status return_code %d\n",return_code);
	return return_code;
}
