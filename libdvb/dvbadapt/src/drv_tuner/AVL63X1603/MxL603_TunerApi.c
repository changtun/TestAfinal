...
#include "MxL603_Debug.h"

/* MxLWare Driver version for MxL603 */
static const UINT8 MxLWare603DrvVersion[] = {1, 1, 2, 1, 0}; 
...
--|
--| RETURN VALUE  : MxL603_MXL_SUCCESS, MxL603_MXL_INVALID_PARAMETER
--|
--|---------------------------------------------------------------------------*/
...
--|                 default state. 
--|
--| RETURN VALUE  : MxL603_MXL_SUCCESS, MXL_FAILED 
--|
--|---------------------------------------------------------------------------*/

MXL_STATUS MxLWare603_API_CfgDevSoftReset(UINT8 devId)
{
/* BEGIN: Modified by zhwu, 2012/12/4 */
  //UINT8 status = MXL_SUCCESS;
  MXL_STATUS status = MXL_SUCCESS;
/* END:   Modified by zhwu, 2012/12/4 */

  //MxL603_MxL_DLL_DEBUG0("%s\n", __FUNCTION__); 
...
--|                 clock out
--|
--| RETURN VALUE  : MxL603_MXL_SUCCESS, MxL603_MXL_INVALID_PARAMETER, MXL_FAILED
--|
--|---------------------------------------------------------------------------*/
...
{
/* BEGIN: Modified by zhwu, 2012/12/4 */
  //UINT8 status = MxL603_MXL_SUCCESS;
  MXL_STATUS status = MXL_SUCCESS;
/* END:   Modified by zhwu, 2012/12/4 */
  UINT8 control = 0;

  //L603_MxL_DLL_DEBUG0("%s", __FUNCTION__); 

  // XTAL freq and cap setting, Freq set is located at bit<5>, cap bit<4:0> 
  // and  XTAL clock out enable <0>
  if ((xtalCfg.xtalFreqSel == MXL603_XTAL_16MHz) || (xtalCfg.xtalFreqSel == MXL603_XTAL_16MHz))
  {
...
/* BEGIN: Modified by zhwu, 2012/12/4 */
  //UINT8 status = MxL603_MXL_SUCCESS;
	MXL_STATUS status = MXL_SUCCESS;
/* END:   Modified by zhwu, 2012/12/4 */
  //MxL603_MxL_DLL_DEBUG0("%s", __FUNCTION__); 
...
/* END:   Modified by zhwu, 2012/12/4 */
  ...
  if (mxlDevVerInfoPtr)
  {
    ...
    //MxL603_MxL_DLL_DEBUG0("Chip ID = 0x%d, Version = 0x%d \n", mxlDevVerInfoPtr->chipId, 
...
/* END:   Modified by zhwu, 2012/12/4 */
  ...
  /*MxL603_MxL_DLL_DEBUG0("%s: Signal Mode = %d, IF Freq = %d, xtal = %d, IF Gain = %d\n", 
                                                __FUNCTION__,
...
/* END:   Modified by zhwu, 2012/12/4 */
  ...
  /*MxL603_MxL_DLL_DEBUG0("%s, AGC sel = %d, attack point set = %d, Flip = %d \n", 
                                                __FUNCTION__, 
                                                agcCfg.agcType,
                                                agcCfg.setPoint, 
                                           ...
                                           MXL603_CHAN_TUNE_CFG_T chanTuneCfg)
{
  ...
  /*MxL603_MxL_DLL_DEBUG0("%s, signal type = %d, Freq = %d, BW = %d, Xtal = %d \n",  
                                              __FUNCTION__,
  ...
  if (chanTuneCfg.startTune == MXL_START_TUNE)
  {
...
MXL_STATUS MxLWare603_API_CfgTunerIFOutParam(UINT8 devId, MXL603_IF_OUT_CFG_T ifOutCfg)
{
  ...
  //MxL603_MxL_DLL_DEBUG0("%s, Manual set = %d \n", __FUNCTION__, ifOutCfg.manualFreqSet); 
...
MXL_STATUS MxLWare603_API_ReqTunerAGCLock(UINT8 devId, MXL_BOOL* agcLockStatusPtr)
{
  ...
  if (agcLockStatusPtr)
  {
    ...
    *agcLockStatusPtr =  lockStatus;
    ...
    //MxL603_MxL_DLL_DEBUG0(" Agc lock = %d\n", (UINT8)*agcLockStatusPtr); 
  }
  else
    status = MXL_INVALID_PARAMETER;
                                                          ...
                                                          MXL_BOOL* refLockPtr)
{
  ...
  if ((rfLockPtr) && (refLockPtr))
  {
    ...
    //MxL603_MxL_DLL_DEBUG0(" RfSynthStatus = %d, RefSynthStatus = %d\n", (UINT8)rfLockStatus,
    //                                                          (UINT8)refLockStatus); 
...
MXL_STATUS MxLWare603_API_ReqTunerRxPower(UINT8 devId, REAL32* rxPwrPtr)
{
  ...
  if (rxPwrPtr)
  {
    ...
    *rxPwrPtr = (REAL32)((tmpData & 0x01FF) >> 2);
    ...
    //MxL603_MxL_DLL_DEBUG0(" Rx power = %f dBm \n", *rxPwrPtr);
  }
  else
    status = MXL_INVALID_PARAMETER;
