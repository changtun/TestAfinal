/**
* \file $Id: drx_driver.h,v 1.97 2010/04/07 03:04:21 lfeng Exp $
*
...
/**
* \brief  DTMB capability flag
*
* Device has DTMB
*
*/
#define DRX_CAPABILITY_HAS_DTMB         (1UL << 14)
...
typedef enum{
   ...
   DRX_UIO_MODE_UNKNOWN   = DRX_UNKNOWN /**< unknown current UIO mode         */
...
#define DRX_CFG_ATV_OUTPUT          ( DRX_CFG_BASE + 18) /* ATV SIF/CVBS output */
#define DRX_CTRL_CFG_MAX            ( DRX_CFG_BASE + 19) /* never to be used  */
...
} DRXAudCarrier_t, *pDRXAudCarrier_t;
...
/**
* \enum DRXAtvOutput_t
* \brief switch between SIF and CVBS output to D/A in test mode
*/
typedef enum
{
   DRX_ATV_OUTPUT_CVBS,
   DRX_ATV_OUTPUT_SIF,
   DRX_ATV_OUTPUT_UNKNOWN = DRX_UNKNOWN
}DRXAtvOutput_t, *pDRXAtvOutput_t;
...
typedef struct {
   ...
   DRXFrequency_t  currentFSRateCorrection;
                                      /**< current FSRate correction          */
...
/**
* /brief DRX preset levels.
*
*/
typedef enum
{
   DRX_PRESET_LEVEL_BASELINE,
   DRX_PRESET_LEVEL_OPEN,
   DRX_PRESET_LEVEL_STANDARD,
   DRX_PRESET_LEVEL_CHANNEL,
   DRX_PRESET_LEVEL_MAX

} DRXPresetLevel_t, *pDRXPresetLevel_t;

/**
* \brief Inner preset function prototype.
*/
typedef void (*DRXPresetFunc_t)(  pDRXDemodInstance_t demod,
               DRXPresetLevel_t   level,
...
#define DRX_STR_MODULE(x) ( \
   ...
   ( x == DRX_MODULE_DRIVERCORE          ) ? "Core Driver"                   : \
   ( x == DRX_MODULE_DEVICEDRIVER        ) ? "Device Driver"                 : \
...
/* Intermediate powermodes for DRXJ2 */
#define DRX_POWER_DOWN_CORE  DRX_POWER_MODE_9
#define DRX_POWER_DOWN_PLL   DRX_POWER_MODE_10
