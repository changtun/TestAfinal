...
static U32 mem_chunks_lock = 0xffffffff;
static GRealMemChunk *mem_chunks = NULL;


#define g_mutex_lock(x)   OS_MutexLock(x)
#define g_mutex_unlock(x)   OS_MutexRelease(x)

#ifdef ENABLE_MEM_PROFILE
static U32 mem_profile_lock=0xffffffff;
...
g_malloc (gulong size)
{
  ...
  p = (gpointer) OS_MemAllocate(system_p, size);/*yangweiqing modify 10-04-26*/
//	p = (gpointer) small_mem_allocate(size);
  if (!p)
	   ...
	   gulong   size)
{
  ...
  if (!mem)
    {
      ...
      p = (gpointer) small_mem_reallocate( NULL, size);/*yangweiqing modify 10-04-26*/
  ...
  //  p = (gpointer) small_mem_allocate(size);
	...
	g_warning ("trying to realloc freed memory\n");
      ...
      //p = (gpointer) small_mem_reallocate (mem, size);/*yangweiqing modify 10-04-26*/
      p = (gpointer) OS_MemReallocate(system_p, mem, size);
    }
  ...
  if (mem)
	...
	g_warning ("freeing previously freed (%lu times) memory\n", *t);
      *t += 1;
      ...
      //small_mem_deallocate( mem);/*yangweiqing modify 10-04-26*/
         OS_MemDeallocate(system_p, mem);
#endif /* ENABLE_MEM_CHECK */
    }
}
		 ...
		 gint    type)
{
  ...
  if(0xffffffff == mem_chunks_lock)
  	OS_MutexCreate(&mem_chunks_lock, "mem_chunks_lock", 0);
...
g_mem_chunk_print (GMemChunk *mem_chunk)
{
 ...
 g_warning("%s: %ld bytes using %d mem areas",
