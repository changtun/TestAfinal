...
static gpointer   g_tree_node_search                (GTreeNode      *node,
						     ...
						     gpointer        data);
static GTreeNode*
g_tree_node_search_num (GTreeNode   *node,
		    GSearchFunc  search_func,gpointer    data,
		    GAuxiliaryFunc  auxiliary_func,gpointer    auxi_data,guint offset,guint num,guint32 *  record);
...
U32   g_tree_global=0xffffffff;  //oxffffffff is elegal value
static GMemChunk *node_mem_chunk = NULL;
static GTreeNode *node_free_list = NULL;

#define G_LOCK(x)    OS_MutexLock((x));
#define G_UNLOCK(x) 	OS_MutexRelease((x))
...
g_tree_new (GCompareFunc key_compare_func)
{
  ...
  if(0xffffffff == g_tree_global)
  	OS_MutexCreate(&g_tree_global,"g_tree_global",0);
	       ...
	       GAuxiliaryFunc  auxiliary_func,gpointer    auxi_data,guint offset,guint num)
{
  ...
  if (rtree->root)
  {
	g_tree_node_search_num (rtree->root,
		      search_func,    data,
		      auxiliary_func,    auxi_data, offset, num,&record);
		    ...
		    GAuxiliaryFunc  auxiliary_func,gpointer    auxi_data,guint offset,guint num,guint32 *  record)
{
  ...
  if(dir == 0)
  {
  	here=g_tree_node_search_num (node->left,search_func,  data, auxiliary_func,auxi_data, offset,num,record);
	if((*record) == (offset+num))
		return here;
	else
	{
		(*record)++;
		if((*record) > offset)
		{
			if(NULL != auxiliary_func)
				auxiliary_func(node->value,auxi_data);
		}
		here=node;
		if((*record) == (offset+num))
			return here;
	}
	here=g_tree_node_search_num (node->right,search_func,  data, auxiliary_func,auxi_data, offset,num,record);
	return here;
...
g_tree_node_check (GTreeNode *node)
{
  ...
  if (node)
	...
	g_warning("g_tree_node_check: failed: %d ( %d )\n",
