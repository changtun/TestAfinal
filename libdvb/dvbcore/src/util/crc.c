...
{
	unsigned int crc;
	CRC_crc32Calculate((unsigned char *)buff, (unsigned int)length,&crc);
	//printf("crc %X\n",crc);
	return crc;
	
}
 
unsigned int tables_crc32(unsigned char *section)
{
	unsigned int	i,
					j,
					crc,
	...
	section_length += 3;
	crc = 0xFFFFFFFF;
	for ( i = 0; i < ((unsigned int)section_length-4); i++ ) {
		temp[0] = crc;
		for ( j = 1; j <= 8; j++ ) {
			bit = (unsigned char)(( (*( s + i )) & (1 << (8 - j))) >> (8 - j) );
			bit &= 1;
			if ( bit ^ ((temp[j-1] >> 31) & 1) ) {
				temp[j] = ( (0xFFFFFFFF & 0x04C11DB7) ^ (temp[j-1] << 1) );
			}
			else {
				temp[j] = (temp[j-1] << 1);
			}
